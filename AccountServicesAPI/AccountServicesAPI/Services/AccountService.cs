    ...
    public class AccountService
    {
        ...
        public List<TransactionDTO> GetStatements(int AccId, DateTime FromDate, DateTime ToDate)
        {
            ...
            try
            {
                ...
                {
                    client.BaseAddress = new Uri("http://localhost:21729/api/");
                    var getTask = client.GetAsync("Transaction/" + AccId);
                    getTask.Wait();
        ...
        public TransactionStatus Withdraw(int AccId, int Amount)
        {
            ...
            try
            {
                ...
                {
                    client.BaseAddress = new Uri("http://localhost:21729/api/");
                    var postTask = client.PostAsync($"Transaction/Withdraw?accountId={AccId}&amount={Amount}", null);
                    postTask.Wait();
        ...
        public TransactionStatus Deposit(int AccId, int Amount)
        {
            ...
            try
            {
                ...
                {
                    client.BaseAddress = new Uri("http://localhost:21729/api/");
                    var postTask = client.PostAsync($"Transaction/Deposit?accountId={AccId}&amount={Amount}", null);
                    postTask.Wait();
                    var result = postTask.Result;
                    if (result.IsSuccessStatusCode)
                    {
        ...
        public TransactionStatus Transfer(int FromAccId, int ToAccId, float Amount)
        {
            TransactionStatus transactionStatus = new TransactionStatus();
            string status = "";
            try
            {
                ...
                {
                    client.BaseAddress = new Uri("http://localhost:21729/api/");
                    var postTask = client.PostAsync($"Transaction/Transfer?sourceAccountId={FromAccId}&targetAccountId={ToAccId}&amount={Amount}", null);
                    postTask.Wait();
                    var result = postTask.Result;
                    if (result.IsSuccessStatusCode)
                    {
